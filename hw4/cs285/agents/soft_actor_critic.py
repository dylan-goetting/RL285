...
class SoftActorCritic(nn.Module):
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        lps = action_distribution.log_prob(samples)
        probs = torch.exp(lps)

        return -torch.mean(probs * lps, axis=0)
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            #tile observation
            obs = obs.unsqueeze(0).repeat(self.num_actor_samples, 1, 1)
            q_values = self.critic(obs, action)
    ...
    def update(
        ...
        if self.target_update_period is not None and self.target_update_period % step == 0:
            self.update_target_critic()
