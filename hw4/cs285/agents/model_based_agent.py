...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        inp = (inp - self.obs_acs_mean)/(self.obs_acs_std + 1e-3)
        pred = self.dynamics_models[i](inp)
        target = ((next_obs - obs) - self.obs_delta_mean)/(self.obs_delta_std + 1e-3)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        self.obs_acs_mean = torch.cat((obs, acs), dim=1).mean(axis=0)
        self.obs_acs_std = torch.cat((obs, acs), dim=1).std(axis=0)
    ...
    def get_dynamics_predictions(
        ...
        pred = self.dynamics_models[i](inp)
        pred = pred * (self.obs_delta_std+1e-3) + self.obs_delta_mean
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        for a in range(self.mpc_horizon):
            acs = action_sequences[:, a, :]
            ...
            next_obs = obs + np.concatenate([np.expand_dims(self.get_dynamics_predictions(i, obs[i], acs),0) for i in range(self.ensemble_size)], axis=0)
            ...
            rewards, _ = self.env.get_reward(next_obs.reshape(-1, self.ob_dim), acs.repeat(self.ensemble_size, 1))
